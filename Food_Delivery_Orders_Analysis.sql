...
select * from(
select *,
ROW_NUMBER() over (partition by cuisine order by no_of_orders desc) as row_num
from cte) as a
where row_num<=3

--2. Find the daily new customer from the launch date(everyday how many new customer we are acquiring)
with cte as(
select Customer_code,cast(min(Placed_at) as date) as first_time_cust
from orders
group by Customer_code)

select first_time_cust,count(*) as no_of_cust
from cte
group by first_time_cust
order by first_time_cust
...
--4. List all the customers with no order in the past 7 days but were acquired one month ago with their first order on promo
with cte as(
...
ROW_NUMBER() over(partition by Customer_code order by Placed_at) as order_num
from orders)
select * from cte
where order_num%3 = 0 and cast(Placed_at as date)='2025-03-31'

--6. List the customers who placed more than 1 order and all thier orders on promo only

select Customer_code,Count(*) as order_num,Count(Promo_code_Name) as promo_order
from orders
group by Customer_code
having count(*)>1 and count(*)=count(Promo_code_name)

--7. what percent of customers were organically acquired in jan 2025(placed their first order without promocode)
with cte as(select *,
ROW_NUMBER() over(partition by Customer_code order by placed_at) as rn
from orders
where MONTH(placed_at)=1 and year(placed_at)=2025)

select count(case when rn=1 and Promo_code_name is null then Customer_code end)*100.0/count(distinct Customer_code)as customer_percent
from cte
